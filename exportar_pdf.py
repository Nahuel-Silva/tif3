...
class Export():
    ...
    def generate_pdf(image1, image2, dist_der, dist_izq, result, name):
        ...
        for img in image1:
            ...
            pdf_file.drawImage(ImageReader(image), 100, 600, width=3*inch, height=3*inch)


        for img in image2:
            image = Image.fromarray(img)
            # Agregar la segunda imagen
            pdf_file.drawImage(ImageReader(image), 300, 600, width=3*inch, height=3*inch)
        ...
        pdf_file.drawString(100, 400, dist_1)
        pdf_file.drawString(100, 300, dist_2)
