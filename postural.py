...
class Postural_change():
    ...
    def shoulders_difference(self, list_mask):
        ...
        for mask in list_mask:
            ...
            # print(cord_centroide)
            
            dist_centroides = np.sqrt((cord_centroide[3][0] - cord_centroide[0][0])**2 + (cord_centroide[3][1] - cord_centroide[0][1])**2)
            original_resolution = 460
            cm_distance = dist_centroides / original_resolution * 2.54

            # # print(pixel_distance)

            # print(f"Distancia del hombro izq a cirtura izq: {cm_distance}cm")
